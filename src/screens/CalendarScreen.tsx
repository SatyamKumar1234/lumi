...
import { Calendar as CalendarIcon, Plus, Scan, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
...
const CalendarScreen = () => {
    ...
    const handleScanDatesheet = async () => {
        try {
            ...
            const newExams = result.map((exam: any, index: number) => ({
                id: Date.now() + index,
    ...
    return (
        <ScrollView style={styles.container} contentContainerStyle={styles.content}>
            ...
            <GlassView style={[styles.calendarCard, { backgroundColor: theme.colors.card }]}>
                ...
                <View style={styles.weekDays}>
                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                        <Text key={i} style={[styles.weekDayText, { color: theme.colors.textSecondary }]}>{d}</Text>
                    ))}
                </View>
            ...
            {exams.map((exam) => (
                <GlassView key={exam.id} style={[styles.examCard, { backgroundColor: theme.colors.card }]}>
