...
const SyllabusScreen = () => {
    ...
    const updateProgress = (subjectId, chapterId, newProgress) => {
    ...
    const calculateTotalProgress = (chapters) => {
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={styles.content}>
            ...
            {isAddingSubject && (<GlassView style={[styles.addSubjectCard, { backgroundColor: theme.colors.card }]}>
                    <TextInput style={[styles.input, { color: theme.colors.text, borderColor: theme.colors.border }]} placeholder="Subject Name" placeholderTextColor={theme.colors.textSecondary} value={newSubjectName} onChangeText={setNewSubjectName}/>
                    <TouchableOpacity onPress={addSubject} style={[styles.saveButton, { backgroundColor: theme.colors.primary }]}>
                        <Text style={{ color: '#FFF', fontWeight: '600' }}>Add</Text>
                    </TouchableOpacity>
                </GlassView>)}

            {subjects.map(subject => (<GlassView key={subject.id} style={[styles.subjectCard, { backgroundColor: theme.colors.card }]}>
                    <TouchableOpacity style={styles.subjectHeader} onPress={() => toggleExpand(subject.id)} activeOpacity={0.7}>
                ...
                <ChevronDown size={20} color={theme.colors.textSecondary}/>}
                    ...
                    {subject.expanded && (<View style={styles.chaptersList}>
                            {subject.chapters.map(chapter => (<View key={chapter.id} style={styles.chapterRow}>
                                    <View style={{ flex: 1 }}>
                        ...
                        styles.progressBarFill,
                        {
                            ...
                            backgroundColor: chapter.progress === 100 ? theme.colors.success : theme.colors.primary
                                    ...
                                    <TouchableOpacity onPress={() => updateProgress(subject.id, chapter.id, chapter.progress === 100 ? 0 : 100)} style={{ marginLeft: 15 }}>
                                        <CheckCircle size={24} color={chapter.progress === 100 ? theme.colors.success : theme.colors.border} fill={chapter.progress === 100 ? theme.colors.success + '20' : 'transparent'}/>
                                    </TouchableOpacity>
                                </View>))}
                            <TouchableOpacity style={[styles.addChapterBtn, { borderColor: theme.colors.border }]} onPress={() => addChapter(subject.id)}>
